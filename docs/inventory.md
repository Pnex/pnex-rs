...
| D6 | **Pas de rétention des artefacts firmware** (décision user 2026-08-19 : le binaire doit rester disponible pour re-flash/recompile dans tous les cas) — les artefacts vivent aussi longtemps que le device. Le volume reste borné par l'upsert par device (1 artefact/device, 1–4 Mo) | ✅ clos |
...
| D10 | **Tokens DRF supprimés** — auth user = JWT IdP uniquement (Keycloak puis Rauthy, cf. D19) ; devices = DeviceToken (inchangés) | Phase 3 |
...
| D17 | **Brick 0 — downlink générique borné au manuel/config** (décision utilisateur 2026-09-02) : le WS `/ws/device` (annonce, set_mode, write, subscribe) sert le provisioning et l'actionnement **manuel** depuis l'UI — le write est un « must » du prototypage rapide sur cartes de dev, mais toujours une action utilisateur (bouton), jamais un automatisme serveur. Toute boucle de régulation reste interdite côté serveur (frontière avec D13 : M2M à l'edge). Même logique : l'artefact firmware générique n'est **jamais recompilé par provisioning** (un `.bin` réutilisable + config injectée au flash) et le workflow d'enregistrement reste celui du wizard existant. Détail : `docs/architecture/brick0.md` | Brick 0 |
| D18 | **Moteur de flow ETL « Node-RED full-Rust » via EdgeLinkd vendored** (PRD 2026-09-03, mode B renforcé — binaire maison) : submodule `vendor/edgelinkd` épinglé à un SHA (jamais patché — extension par nœuds custom + binaire `pnex-flow-runtime` qui lie `edgelink-core` en features `core,js`) ; runtime headless supervisé par Loco (process enfant, SIGUSR1 = rechargement à chaud via `Engine::redeploy_flows`, zéro surface HTTP — l'éditeur Node-RED embarqué jamais exposé) ; source de vérité Postgres (`flows` + `flow_versions` append-only, `deployed_version_id`, concurrence optimiste 409 — écart assumé vs 400) ; `flows.json` = simple artefact projeté à chaque deploy (tous les flows `deployed` de l'instance, multi-tabs, `pnex_flow_id`/`pnex_version` embarqués) ; secrets par env allowlist uniquement (crédentials Node-RED inexistants côté EdgeLinkd) ; schéma unique des nœuds dans `pnex-core` (validation structurelle + contrats typés aux frontières, pas de type-check global) ; premier nœud custom `pnex-sql` (SELECT-only au build, sqlx Postgres). Ingestion uniquement — frontière D13/D17 inchangée. Détail : `docs/architecture/flow-engine.md`, `vendor/README.md` | Phase 0+1 (runtime+SQL) ; Phases 2-5 (WASM, IoT/O2, alerting, éditeur) |
| D19 | **IdP Rauthy (full-Rust) remplace Keycloak** (décision utilisateur 2026-09-03) : dernier composant non-Rust de la stack dev → `ghcr.io/sebadob/rauthy` avec sa DB embarquée **hiqlite** (volume `pnex-rauthydata`). Client `pnex` public PKCE S256 **RS256** conservé (zéro changement du verifier JWKS backend) ; proxy OAuth2 backend inchangé (password grant dev OK — username = **email** chez Rauthy) ; access tokens lean (pas de `preferred_username` → `Claims` assoupli, `display_name` retombe sur l'email) ; `users.keycloak_uuid` → `users.idp_sub` (Rauthy émet des `sub` 24 chars, pas des UUID — **reset DB assumé**, app pas en prod) ; register/reset = pages UI Rauthy (`/auth/v1/users/register`, `/auth/v1/account`), inscription ouverte + catcher SMTP **mailcrab** ; refresh tokens Rauthy `nbf = exp_AT − 60` (refresh immédiat rejeté par design — l'UI ne rafraîchit que sur 401) ; config : `deploy/rauthy/` | 2026-09-03 |
...
| /api/v1/telemetry/catalog, /series | **FAIT (2026-08-19)** — `controllers/visualization.rs` + `services/visualization.rs` : **la manière formalisée de requêter O2** (à la InfluxDB) — catalogue des séries (`/streams` + `last_over_time(nom[24h])` par métrique, une série par device) et points d'une série (`query_range` nom + égalité `{device_id="…"}`, fenêtres preset 1h/6h/24h, step = fenêtre/120, cap 240 points) ; passcode→fallback root, timeout 5 s, dégradation `available:false` ; **anti-injection PromQL** : charset fermé métrique/device + fenêtre preset → 400 avant toute construction de requête ; page Visualisation (chart SVG maison, ≤ 6 séries, polling 15 s) | ✅ |
