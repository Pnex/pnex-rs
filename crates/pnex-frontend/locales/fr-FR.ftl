...
devices-build-now = Compiler le firmware maintenant
devices-build-ssid = SSID WiFi
devices-build-wifi-password = Mot de passe WiFi
devices-build-server-url = Serveur PNEX (hôte joint par le device)
devices-build-status = Build firmware lancé (en file) — suivi sur la page Build firmware.
