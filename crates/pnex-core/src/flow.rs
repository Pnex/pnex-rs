...
/// coordonnée d'accès ici — l'org O2 réelle est estampillée dans l'artefact
/// au deploy (`pnex_o2_org`) et les creds viennent de l'env du runtime.
#[derive(Debug, Clone, Default, PartialEq, Serialize, Deserialize)]
            ...
            for t in &w.targets {
                if !seen.contains(t) {
                    v.push(FlowViolation::new(
                        ...
                        "dangling_target",
                        format!(
                            "le port {} du nœud « {} » cible un nœud inconnu : « {} »",
                            w.port, n.id, t
                        ),
                    ));
                }
...
pub fn validate_sql_readonly(query: &str) -> Result<(), FlowViolation> {
    ...
    const FORBIDDEN: [&str; 10] = [
        "INSERT", "UPDATE", "DELETE", "MERGE", "TRUNCATE", "ALTER", "CREATE", "DROP", "GRANT",
        "COPY",
    ];
...
mod tests {
    ...
    fn sql_readonly_rejecte_ecriture() {
        assert_eq!(
            validate_sql_readonly("DELETE FROM t").unwrap_err().code,
            "readonly_sql"
        );
        ...
        assert_eq!(
            validate_sql_readonly("UPDATE t SET x = 1")
                .unwrap_err()
                .code,
            "readonly_sql"
        );
        assert_eq!(
            validate_sql_readonly("INSERT INTO t VALUES (1)")
                .unwrap_err()
                .code,
            "readonly_sql"
        );
    ...
    fn sql_readonly_rejecte_multi_instructions_et_vide() {
        ...
        assert_eq!(
            validate_sql_readonly("   ").unwrap_err().code,
            "empty_query"
        );
        assert_eq!(
            validate_sql_readonly("GRANT ALL ON t TO x")
                .unwrap_err()
                .code,
            "readonly_sql"
        );
    ...
    fn sql_contrat_frontiere() {
        ...
        assert_eq!(
            SqlQueryRequest::validate_payload(Some(&json!(42)), req.as_slice())
                .unwrap_err()
                .code,
            "sql_input_contract"
        );
    ...
    fn validate_device_calc_metric() {
        ...
        let mut g = FlowGraph {
            nodes: vec![node_device_reads("d1", &[])],
        };
    ...
    fn display_sans_config_est_valide() {
        ...
        let g: FlowGraph =
            serde_json::from_str(r#"{"nodes":[{"id":"n1","kind":"display"}]}"#).unwrap();
    ...
    fn dto_create_flow_deserialise() {
        ...
        let u: UpdateFlow =
            serde_json::from_str(r#"{"expected_version_number": 2, "graph": {"nodes": []}}"#)
                .unwrap();
