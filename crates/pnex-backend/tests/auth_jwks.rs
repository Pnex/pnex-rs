...
//! RS256, kid inconnu) contre un mock JWKS — pas de Keycloak requis.
...
async fn token_valide_passe_et_expose_les_claims() {
    ...
    let token = mint_token(&TokenSpec {
        issuer: format!("{base}/realms/pnex-realm"),
        ..Default::default()
    });
...
async fn mauvais_issuer_rejete() {
    ...
    let v = verifier(&base).await;
    let token = mint_token(&TokenSpec {
        // Token signé par la bonne clé mais émis par un autre realm.
        issuer: "http://evil.example/realms/other".into(),
        ..Default::default()
    });
    assert!(matches!(
...
async fn token_non_rs256_rejete() {
    ...
    let claims = serde_json::json!({
        ...
        "iss": format!("{base}/realms/pnex-realm"),
        "aud": ["pnex"],
        "exp": chrono::Utc::now().timestamp() + 3600,
    });
